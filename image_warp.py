...
def image_warp(images, flow, name='image_warp'):
    with tf.name_scope(name):
        ...
        coords = tf.stack([tf.minimum(tf.maximum(tf.cast(0.0, coords.dtype), coords[:, :, 0]), tf.cast(height, flow.dtype) - 1.0),
                           tf.minimum(tf.maximum(tf.cast(0.0, coords.dtype), coords[:, :, 1]), tf.cast(width, flow.dtype) - 1.0)], axis=2)

        floors = tf.cast(tf.floor(coords), tf.int32)
        ceils = floors + 1
        alphas = tf.cast(coords - tf.cast(floors, flow.dtype), images.dtype)
        alphas = tf.reshape(tf.minimum(tf.maximum(tf.cast(0.0, alphas.dtype), alphas), 1.0), shape=[batch_size, height, width, 1, 2])
        ...
        def gather(y_coords, x_coords):
            ...
            # print('************** images_flattened : ', images_flattened)
